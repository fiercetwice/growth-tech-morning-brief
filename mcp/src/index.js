...
function buildServer(env) {
  ...
  server.registerTool('get_stock_snapshot', {
    description: 'Return deterministic stock setup, SEC fundamentals, valuation percentile, and target model without AI.',
  ...
  server.registerTool('analyze_watchlist', {
    description: 'Deep-analyze every supplied unique ticker; failures are isolated per symbol.',
    inputSchema: {
