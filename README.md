...
Each provider returns bounded structured research packets. The final Markdown is not model-generated: deterministic code renders and validates the exact five-section product schema—Executive Summary, Overnight and Market Context, AI Cycle Dashboard, Sector Scorecard, and Watchlist. Executive Summary is capped at five labeled bullets; the two dashboards contain every configured row; the Watchlist table contains every core symbol with a uniform price/change/range/valuation/catalyst/risk/action/research-status contract. Buy/Sell calls require a material fresh event or same-day earnings, while a large move without material news is Watch-only. Without supplied holdings and target weights, `Trim` is removed from the action enum. The validator also rejects unsupported demand, CapEx-cycle, institutional-flow, portfolio, and balance-sheet claims.
