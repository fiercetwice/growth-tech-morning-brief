# Growth Tech Morning Brief v0.5.8 — target and mispricing engine
...
- Deterministic Target & Mispricing Engine: when the SEC cache is fresh and history is sufficient, median-normalize the most recent four filing-vintage TTM EPS/share or revenue/share observations and apply historical trailing-multiple P25/P50/P75 to produce bear/base/bull implied values. The output includes base upside, downside to bear, risk/reward, preferred entry, method, assumptions, confidence, and a -2 to +2 valuation adjustment.
...
The `.github/workflows/cloudflare-worker.yml` workflow runs `npm test` for pull requests, pushes to `main`, and manual dispatches. Successful pushes to `main` and manual dispatches deploy the Worker with `npm run deploy` using the existing `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` repository secrets. Before the post-deploy report test, the workflow waits until `/health` reports the exact engine version and build revision. It then calls the build-specific `/run-report/v0.5.8/build/0.5.8` endpoint and retries only a 404 response, so a still-propagating older Worker cannot generate or overwrite the new report artifact.
...
Each provider returns bounded structured research packets. The final Markdown is not model-generated: deterministic code renders and validates the exact five-section product schema—Executive Summary, Overnight and Market Context, AI Cycle Dashboard, Sector Scorecard, and Watchlist. Executive Summary is capped at five labeled bullets, including `Research Exclusions` for screening results that are explicitly not portfolio recommendations; the two dashboards contain every configured row; the Watchlist table contains every core symbol with a uniform price/change/range/valuation/target/catalyst/risk/final-action/research-status contract. `Final Action` is the single post-gate action source: discovery Buy/Sell calls require a material fresh event or same-day earnings, while a core value call may instead use the qualified 1Q/2Q rerating-path exception described below. A large move without either form of evidence is Watch-only. The Sector Scorecard uses non-transactional `Favorable`/`Neutral`/`Cautious` stances rather than Buy/Sell actions and excludes expired SEC growth observations from fundamentals ratings while disclosing all stale members, as well as fresh members whose reported growth is unavailable. Valuation cells identify trailing multiples explicitly and mark unavailable forward estimates as unavailable. Without supplied holdings and target weights, `Trim` and `Review position size` are forbidden. The validator also rejects unsupported demand, CapEx-cycle, institutional-flow, portfolio, and balance-sheet claims.
...
v0.5.8 adds a deterministic target layer without adding a market-data provider. Positive TTM EPS uses historical P/E; otherwise the engine falls back to P/S. Fewer than 126 valuation observations, fewer than two filing vintages, stale SEC inputs, or missing price data return `Target unavailable` with a reason. The preferred-entry price is base value divided by 1.20, and risk/reward is base-value upside divided by downside to bear value when both are positive. Base upside maps to `+2` at 30% or more, `+1` at 20–30%, `0` at 10–20%, `-1` at 5–10%, and `-2` below 5%.

Targets are calculated for ranking before research, but only `researchSymbols` may receive them in the final report; context-only rows are forced to `Target unavailable`. A Medium/High-confidence target is required for Buy actions. A catalyst-backed `Buy now` requires at least neutral 10% base upside; 5–10% is deterministically downgraded to `Buy on weakness`, and below 5% remains `Watch`. A core name may clear the catalyst gate without same-day news only with at least 20% base upside, complete financial evidence, and a testable 1Q/2Q rerating path; discovery names still require a verified fresh catalyst. The dated snapshot and research packet preserve the target, action, confidence, catalyst state, and data-completeness state for later 1D/5D/1M/3M calibration. SPY is identified as the market benchmark; sector benchmark mapping and realized outcomes remain pending rather than being invented.
