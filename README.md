...
- Absolute opportunity gate: material fresh events, same-day earnings, moves of at least 3%, or extreme valuation/range risk flags may reach structured AI research. Buy/Sell still require a verified catalyst. Without supplied holdings and target weights, `Trim` is not an allowed action; extreme valuation produces `Review position size` only.
...
Each provider returns bounded structured research packets. The final Markdown is not model-generated: deterministic code renders and validates the exact five-section product schema—Executive Summary, Overnight and Market Context, AI Cycle Dashboard, Sector Scorecard, and Watchlist. Executive Summary is capped at five labeled bullets; the two dashboards contain every configured row; the Watchlist table contains every core symbol with a uniform price/change/range/valuation/catalyst/risk/final-action/research-status contract. `Final Action` is the single post-gate action source: Buy/Sell calls require a material fresh event or same-day earnings, while a large move without material news is Watch-only. The Sector Scorecard uses non-transactional `Favorable`/`Neutral`/`Cautious` stances rather than Buy/Sell actions. Valuation cells identify trailing multiples explicitly and mark unavailable forward estimates as unavailable. Without supplied holdings and target weights, `Trim` is removed from the action enum. The validator also rejects unsupported demand, CapEx-cycle, institutional-flow, portfolio, and balance-sheet claims.
